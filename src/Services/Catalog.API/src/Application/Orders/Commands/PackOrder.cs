...
public record PackOrderCommand(int Number) : IRequest<Result<OrderDetailDto>>;

public class PackOrderHandler(AppDbContext db, IPublisher publisher) : IRequestHandler<PackOrderCommand, Result<OrderDetailDto>>
{
    public async Task<Result<OrderDetailDto>> Handle(PackOrderCommand request, CancellationToken ct)
    {
