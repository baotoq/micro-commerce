...
public record ShipOrderCommand(int Number, string Carrier, string? TrackingNote) : IRequest<Result<OrderDetailDto>>;

public class ShipOrderHandler(AppDbContext db, IPublisher publisher) : IRequestHandler<ShipOrderCommand, Result<OrderDetailDto>>
{
    public async Task<Result<OrderDetailDto>> Handle(ShipOrderCommand request, CancellationToken ct)
    {
        ...
        try
        {
            order.Ship(request.Carrier, request.TrackingNote);
        }
