...
public record ActivatePromotionCommand(string Code) : IRequest<Result<PromotionDto>>;

public class ActivatePromotionHandler(AppDbContext db, IPublisher publisher) : IRequestHandler<ActivatePromotionCommand, Result<PromotionDto>>
{
    public async Task<Result<PromotionDto>> Handle(ActivatePromotionCommand request, CancellationToken ct)
    {
