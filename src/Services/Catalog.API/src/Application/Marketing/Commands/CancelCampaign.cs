...
public record CancelCampaignCommand(Guid Id) : IRequest<Result<CampaignDto>>;

public class CancelCampaignHandler(AppDbContext db, IPublisher publisher) : IRequestHandler<CancelCampaignCommand, Result<CampaignDto>>
{
    public async Task<Result<CampaignDto>> Handle(CancelCampaignCommand request, CancellationToken ct)
    {
