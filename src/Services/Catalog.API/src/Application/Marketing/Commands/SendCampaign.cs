...
public record SendCampaignCommand(Guid Id) : IRequest<Result<CampaignDto>>;

public class SendCampaignHandler(AppDbContext db, IPublisher publisher) : IRequestHandler<SendCampaignCommand, Result<CampaignDto>>
{
    public async Task<Result<CampaignDto>> Handle(SendCampaignCommand request, CancellationToken ct)
    {
