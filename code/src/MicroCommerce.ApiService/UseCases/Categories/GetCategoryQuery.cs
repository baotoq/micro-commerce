...
public record GetCategoryQuery : IRequest<CategoryViewModel>
{
    ...
    public static Func<IMediator, string, Task<CategoryViewModel>> EndpointHandler => (mediator, id) => mediator.Send(new GetCategoryQuery(id));
}

public class GetCategoryQueryHandler(ApplicationDbContext context) : IRequestHandler<GetCategoryQuery,CategoryViewModel>
{
