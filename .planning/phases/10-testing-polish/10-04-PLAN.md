...
must_haves:
  truths:
    ...
    - "MassTransit replaced with InMemoryTestHarness to avoid Service Bus dependency"
  ...
  <action>
    **ApiWebApplicationFactory.cs:**
    ...
    2. In `ConfigureWebHost`, override services to:
       ...
       c. Replace any Azure Blob Storage / Keycloak services that would fail without real infrastructure. Use `services.RemoveAll<T>()` for any cloud-dependent services. Register no-op or in-memory stubs if needed.
